 ...
 *   5. wrangler deploy               部署后端 → api.example.com
 *   6. wrangler pages deploy         部署前端 → example.com（Pages 项目 marytopens）
 *
 * 用法（在 worker/ 目录）：
 *   npm run deploy
 */
...
// 5. 部署后端
console.log('\n[5/6] 部署 Worker 后端 → api.example.com ...');
run(['deploy']);

// 6. 部署前端
console.log('\n[6/6] 部署 Pages 前端 → example.com ...');
run(['pages', 'deploy', pagesDir, '--project-name', 'marytopens', '--branch', 'main']);

// 7. 验证后端连通性（核心：确认 api.example.com 真正在响应，避免「无法连接后端API」）
console.log('\n[7/7] 验证后端连通性 https://api.example.com/api/health ...');
try {
  const hc = await fetch('https://api.example.com/api/health');
  ...
  console.warn('  ! 暂时无法连接到 api.example.com：', String(e).slice(0, 200));
  console.warn('    可能是自定义域 api.example.com 尚未在 Cloudflare 完成解析/激活（DNS 通常需几分钟生效）。');
  console.warn('    请到 Cloudflare 控制台 → Workers → marytopens-api → 路由 确认 api.example.com 已绑定。');
}

console.log('\n=== 部署完成 ===');
console.log('请到 Cloudflare 控制台完成收尾：');
console.log('  1. Workers → marytopens-api → Settings → 自定义域：绑定 api.example.com（路由已在 wrangler.toml，等待 DNS 生效）');
console.log('  2. Pages → marytopens → 自定义域：绑定 example.com（根域）');
...
console.log('  4. 验证：curl https://api.example.com/api/meta');
