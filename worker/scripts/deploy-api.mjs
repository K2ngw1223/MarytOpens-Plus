 ...
 *   6. PUT  /workers/domains                 绑定 api.example.com
 *   7. GET  https://api.example.com/api/health 连通性自检
 */
...
const db1Binding = pick(/\[\[d1_databases\]\][\s\S]*?binding\s*=\s*"([^"]+)"/) || 'DB1';
const db1Id = pick(/\[\[d1_databases\]\][\s\S]*?database_id\s*=\s*"([0-9a-f-]{36})"/);
...
// ---------------- 1.5 拉取线上现有变量，避免用 wrangler.toml 占位符覆盖真实值 ----------------
// 全量 PUT 会替换所有 bindings；wrangler.toml 里 GitHub/Discord/CF/Turnstile 等 ID 多为占位符，
// 若直接上传会把线上真实值覆盖成占位符，导致登录/绑定失效。策略：线上真实值优先于占位符。
try {
  const live = cf('GET', `/accounts/${ACCOUNT}/workers/scripts/${scriptName}/settings`);
  const liveVars = {};
  if (live && live.success && Array.isArray(live.result && live.result.bindings)) {
    for (const b of live.result.bindings) {
      if (b.type === 'plain_text' && typeof b.text === 'string') liveVars[b.name] = b.text;
    }
  }
  const isPH = (v) => !v || /^(在此填入|your_|<|xxx)/i.test(v);
  for (const [k, v] of Object.entries(liveVars)) {
    if (!(k in vars) || isPH(vars[k])) vars[k] = v; // 线上有真实值则保留，避免被占位符覆盖
  }
  console.log(`· 已合并线上 ${Object.keys(liveVars).length} 个变量（占位符以线上真实值兜底）`);
} catch (e) {
  console.warn('  ! 拉取线上绑定失败，将仅使用 wrangler.toml 变量：' + (e && e.message));
}
...
console.log(`· D1 绑定     : ${db1Binding} → ${db1Id || '(未配置)'}`);
...
const bindings = [
  ...
  ...(db1Id ? [{ type: 'd1', name: db1Binding, id: db1Id }] : []),
