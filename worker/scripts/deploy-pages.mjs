 ...
 *
 *   --takeover  把根域 example.com 从占用它的 Worker 上解绑，改绑到本 Pages 项目
 *
...
const DOMAIN = process.env.PAGES_DOMAIN || 'example.com';
...
// 每次部署生成唯一缓存戳，自动给 /assets/ 引用加上 ?v=<buster>，防止 Pages 边缘/浏览器缓存导致旧版 JS/CSS 残留
const ASSET_CACHE_BUSTER = Date.now().toString(36).slice(0, 10);
function bustAssets(content, ext) {
  const s = content.toString('utf8');
  if (ext === 'css') {
    return Buffer.from(s.replace(/url\((['"]?)(\/assets\/[^)'"?]+)\1\)/g, (m, q, path) => {
      return `url(${q}${path}?v=${ASSET_CACHE_BUSTER}${q})`;
    }));
  }
  // html / js / mjs：给引号包裹的 /assets/... 加 query
  return Buffer.from(s.replace(/(['"])(\/assets\/[^"'?]+?)\1/g, (m, q, path) => {
    return `${q}${path}?v=${ASSET_CACHE_BUSTER}${q}`;
  }));
}
...
const entries = files.map((abs) => {
  ...
  // 给 HTML/JS/CSS 里的 /assets/ 引用加缓存戳，彻底避免旧版静态资源被客户端/边缘缓存钉死
  if (['html','js','css','mjs'].includes(ext)) content = bustAssets(content, ext);
