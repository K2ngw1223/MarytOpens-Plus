...
浏览器 ──https://example.com──▶ Cloudflare Pages (静态前端 pages/)
        ──https://api.example.com/api/*──▶ Worker (worker/src/index.js)
                                          ├─ D1 绑定  DB1  （主存储 · KV 兼容层）
                                          ├─ KV 绑定  DB   （file:* 二进制回退）
                                          └─ R2 绑定  MEDIA（头像/配图 · 可选）
```
...
| `example.com` 域名 | 已添加到 Cloudflare，NS 指向 Cloudflare |
| Node.js ≥ 18 | 本地运行 `wrangler` 与工具脚本 |
...
# D1 数据库（主存储）
wrangler d1 create marytopens
# 记下 database_id，填入 wrangler.toml 的 [[d1_databases]]

# R2 桶（可选）
wrangler r2 bucket create marytopens-media
```

> **D1 表结构需手动创建**（Worker 不会自动建表）。建表语句见
> [`KV_SCHEMA.md`](KV_SCHEMA.md)，未建表时首个请求会直接报错。
...
2. 把 `[[d1_databases]]` 的 `database_id` 替换为上一步拿到的真实值。
   > 嫌麻烦的话，先跑 `node tools/configure.mjs`，域名类变量会自动填好；资源 ID 仍需手填。
3. `[vars]` 中补全非敏感配置：
...
1. 打开 `https://example.com`，进入登录/注册。
2. 使用 `wrangler secret` 中设置的 `SUPER_ADMIN_EMAIL` + `SUPER_ADMIN_PASSWORD` 登录（首次启动自动创建该账号并授予 `site.admin`）。
3. 登录后访问 `https://example.com/admin_dashboard.html` 进入管理后台，按需配置站点标题、导航、关于页、身份组等。
...
- 在对应开发者后台创建 OAuth App，回调地址填 `https://api.example.com/api/oauth/<provider>/callback`。
...
平台内置机器人鉴权通道（`X-Bot-Token`）。在**管理后台 → 机器人**面板创建令牌，
按需授予 `post.create` / `post.delete.any` 等 scope，即可用任意语言/框架对接：

```bash
curl -X POST https://api.example.com/api/posts \
  -H "X-Bot-Token: <你在后台签发的令牌>" \
  -H "Content-Type: application/json" \
  -d '{"title":"来自机器人的帖子","content":"..."}'
```

> 本仓库不包含具体的机器人客户端实现（例如 Discord 桥接）。
> scope 与人类权限位是同一套枚举，详见 [`PERMISSIONS.md`](PERMISSIONS.md#5-机器人服务账号的权限)。
...
MAIL_API_URL  = "https://mail.example.com/api/send"
...
2. **回调地址（Redirect URI）**填：`https://api.example.com/api/cf/bind/callback`
...
- **下载页 URL（本网站下载）**（默认 `https://example.com/downloads/files`）：落地页显示「本网站下载」按钮。该地址应指向独立部署的 Desktop / Android APK 下载站（属于另一个 Workers + Pages 项目，**本仓库不包含客户端源码**，写完客户端后再启用）。
...
| `MEDIA_PUBLIC_BASE` | vars | R2 公开访问基址（默认 `https://api.example.com/files`） |
...
```bash
# 1) 后端元信息
curl -s https://api.example.com/api/meta | head
...
# 4) 机器人通道自检（若已在后台签发令牌）
curl -s https://api.example.com/api/auth/me -H "X-Bot-Token: <你的令牌>"
```
