 ...
 *
 * 为什么需要它：
 *   页面内联脚本习惯用 `const { $, $$, esc } = App;` 取工具函数。
 *   一旦漏解构某个名字，调用时抛的是 ReferenceError —— 而且往往抛在
 *   try/catch 之外，表现为「按钮点了没反应，后端日志里连请求都没有」。
 *   这类静默失败在浏览器控制台之外很难发现，所以用脚本挡在部署之前。
 *
 * 规则：内联脚本裸调用了 App 导出的成员，但既没有
 *   ① 从 App 解构，② 写 App.xxx，③ 也没有本地定义同名函数
 *   即判为问题。
 */
const fs = require('fs');
